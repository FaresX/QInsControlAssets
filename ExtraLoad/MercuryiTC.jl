...
MercuryiTC_angleset_set(instr, val) = query(instr, "SET:DEV:DB4.G1:AUX:SIG:Target Angle:$val")
MercuryiTC_angleset_get(instr) = rstrip(split(query(instr, "READ:DEV:DB4.G1:AUX:SIG:Target Angle"), "Target Angle:")[end], '\xb0')

MercuryiTC_angleread_get(instr) = rstrip(split(query(instr, "READ:DEV:DB4.G1:AUX:SIG:Angular Position"), "Angular Position:")[end], '\xb0')